...
self.BJSpell = (function () {
   ...
   * @constructor
   */
  function BJSpell(lang, callback) {
    if (this instanceof BJSpell) {
      var self = this;
      if (!callback) callback = empty;
      if (BJSpell[lang]) {
        if (BJSpell[lang] instanceof BJSpell) self = BJSpell[lang];
        else BJSpell[lang] = sync(self, BJSpell[lang]);
        self.checked = {};
        var keys = (self.keys = []),
          words = self.words,
          i = 0,
          key;
        for (key in words) keys[i++] = key;
        ...
        BJSpell[lang] = self;
        BJSpell.call(self, lang, callback);
      }
      return self;
    } else return new BJSpell(lang, callback);
  }
  ...
  BJSpell.prototype.suggest = function (word, many) {
    if (typeof this.words[word] === "string") {
      ...
      if (-1 === keys.indexOf(word)) keys[length] = word;
      keys.sort();
      ...
      if (length !== keys.length) keys.splice(indexOf, 1);
    }
    return words;
  };
